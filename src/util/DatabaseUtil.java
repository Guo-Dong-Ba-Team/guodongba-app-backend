...
//        return DriverManager.getConnection("jdbc:mysql://localhost/guodongba", "root", "");
    ...
    public static ResultSet getOrder(String phone, String status)
            throws SQLException
    {
        initConnection();
        Statement statement = connection.createStatement();

        String sentence = "SELECT user, gym_id, time, status, money FROM order_info WHERE user = " + phone + " AND status = "+ status + ";";
        ResultSet resultSet = statement.executeQuery(sentence);
